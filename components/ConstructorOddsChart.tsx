...
import type { TeamPriceHistory } from "@/types";

interface ConstructorOddsChartProps {
  oddsHistory: TeamPriceHistory[];
}
...
function buildChartData(teams: TeamPriceHistory[]) {
  const timeSet = new Set<number>();
  for (const team of teams) {
    for (const pt of team.history) {
      timeSet.add(pt.t);
    }
  }
  ...
  return times.map((time) => {
    ...
    for (let i = 0; i < teams.length; i++) {
      ...
      row[teams[i].teamId] = lastValues[i];
    }
    return row;
  });
}
...
export function ConstructorOddsChart({ oddsHistory }: ConstructorOddsChartProps) {
        ...
        {oddsHistory.map((team, i) => (
          <div key={team.teamId} className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-foreground">
              {Math.round(team.currentProbability * 100)}%
            </span>
          </div>
        ))}
      </div>
